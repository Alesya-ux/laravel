...
<article class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border border-gray-100 overflow-hidden">
    ...
    <div class="relative h-48 overflow-hidden bg-gray-50">
        ...
        <!-- Вариант с контрастным текстом и обводкой -->
        <div class="absolute bottom-4 left-4 right-4">
            <h3 class="text-xl font-bold text-white mb-1" style="text-shadow: 2px 2px 4px rgba(0,0,0,0.8), -1px -1px 2px rgba(0,0,0,0.8);">{{ $catalog->name }}</h3>
            @if($catalog->description)
                <p class="text-sm text-white line-clamp-2" style="text-shadow: 1px 1px 3px rgba(0,0,0,0.8);">{{ Str::limit($catalog->description, 80) }}</p>
            @endif
        </div>
