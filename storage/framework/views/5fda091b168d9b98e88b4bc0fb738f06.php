    ...
    <main>
        ...
        <section class="rounded-lg bg-neutral-50 max-w-[95%] mx-auto shadow-lg p-10 mt-10 fade-in">
            ...
            <div class="relative mt-6">
                <!-- Кнопки навигации -->
                <button class="catalog-nav-btn prev absolute left-2 top-1/2 transform -translate-y-1/2 z-10 bg-white/80 hover:bg-white border border-gray-300 rounded-full w-10 h-10 flex items-center justify-center shadow-lg transition-all duration-300 hover:shadow-xl" id="catalogPrevBtn">
                    <svg class="w-5 h-5 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                    </svg>
                </button>
                <button class="catalog-nav-btn next absolute right-2 top-1/2 transform -translate-y-1/2 z-10 bg-white/80 hover:bg-white border border-gray-300 rounded-full w-10 h-10 flex items-center justify-center shadow-lg transition-all duration-300 hover:shadow-xl" id="catalogNextBtn">
                    <svg class="w-5 h-5 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Контейнер карусели -->
                <div class="relative w-full overflow-hidden py-3" id="catalogContainer">
                    <div class="flex transition-transform duration-300 ease-in-out" id="catalogTrack">
                            ...
                            <div class="catalog-slide flex-shrink-0 w-[calc(20%-0.1rem)] min-w-[160px] xl:w-[calc(20%-0.1rem)] lg:w-[calc(25%-0.1rem)] md:w-[calc(33.333%-0.1rem)] sm:w-[calc(50%-0.1rem)] xs:w-[calc(100%-0.1rem)]">
                                <div class="card bg-white shadow-lg transition-all duration-300 group mx-0.5 h-[420px] md:h-[380px] sm:h-[400px] hover:-translate-y-2 hover:shadow-2xl">
                                    <figure class="relative overflow-hidden">
                                        <img src="/storage/<?php echo e($catalog->picture); ?>" alt="<?php echo e($catalog->name); ?>" class="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300"/>
                                    </figure>
                                    <div class="card-body p-4">
                                        <h3 class="text-xl font-semibold text-gray-800 mb-2 line-clamp-2"><?php echo e($catalog->name); ?></h3>
                                        <p class="text-sm text-gray-600 mb-4 line-clamp-3"><?php echo e($catalog->description); ?></p>
                                        <div class="card-actions justify-end">
                                            <a href="/catalog/<?php echo e($catalog->id); ?>" class="btn btn-primary bg-cyan-700 hover:bg-cyan-600 text-white border-0 transition-all duration-300 hover:scale-105">
                                                Подробнее
                                            </a>
                                        </div>
                                    </div>
                ...
                <!-- Индикаторы позиции -->
                <div class="flex justify-center mt-4 space-x-2" id="catalogIndicators">
                    <!-- Индикаторы будут добавлены динамически через JavaScript -->
                </div>
            </div>
        </section>
    </main>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    */
    ...
    const catalogIndicators = document.getElementById('catalogIndicators');
    ...
    function updateSlidesPerView() {
        ...
        totalPages = Math.ceil(catalogSlides.length / slidesPerView);
        console.log(`Ширина экрана: ${width}px, видимых слайдов: ${slidesPerView}, всего страниц: ${totalPages}`);
    }
    ...
    function goToCatalogSlide(slideIndex) {
        ...
        console.log(`Переход к странице каталога: ${slideIndex + 1}/${totalPages}`);
    }
    
    // Функция для обновления индикаторов
    function updateIndicators() {
        if (!catalogIndicators) return;
        
        catalogIndicators.innerHTML = '';
        
        for (let i = 0; i < totalPages; i++) {
            const indicator = document.createElement('button');
            indicator.className = `w-3 h-3 rounded-full transition-all duration-300 ${
                i === currentCatalogSlide 
                    ? 'bg-cyan-700 scale-125' 
                    : 'bg-gray-300 hover:bg-gray-400'
            }`;
            indicator.addEventListener('click', () => goToCatalogSlide(i));
            catalogIndicators.appendChild(indicator);
        }
    }
    ...
    function updateNavButtons() {
        if (catalogPrevBtn) {
            catalogPrevBtn.disabled = currentCatalogSlide === 0;
            catalogPrevBtn.style.opacity = currentCatalogSlide === 0 ? '0.5' : '1';
        }
        
        if (catalogNextBtn) {
            catalogNextBtn.disabled = currentCatalogSlide === totalPages - 1;
            catalogNextBtn.style.opacity = currentCatalogSlide === totalPages - 1 ? '0.5' : '1';
        }
    }
    
    // Функция для следующего слайда
    function nextCatalogSlide() {
        const nextSlide = (currentCatalogSlide + 1) % totalPages;
        goToCatalogSlide(nextSlide);
    }
    
    // Функция для предыдущего слайда
    function prevCatalogSlide() {
        const prevSlide = (currentCatalogSlide - 1 + totalPages) % totalPages;
        goToCatalogSlide(prevSlide);
    }
    
    // Обработчики событий для кнопок навигации
    if (catalogNextBtn) {
        catalogNextBtn.addEventListener('click', function(e) {
            e.preventDefault();
            nextCatalogSlide();
        });
    }
    ...
    if (catalogPrevBtn) {
        catalogPrevBtn.addEventListener('click', function(e) {
            e.preventDefault();
            prevCatalogSlide();
        });
    }
    ...
    function startAutoPlay() {
        ...
        autoPlayInterval = setInterval(() => {
            nextCatalogSlide();
        }, 3000); // 3 секунды - быстрее чем было
    }
    ...
    if (catalogTrack && catalogSlides.length > 0) {
        updateSlidesPerView();
        goToCatalogSlide(0);
        console.log(`Карусель каталога инициализирована: ${catalogSlides.length} слайдов, ${slidesPerView} видимых, ${totalPages} страниц`);
        ...
        catalogTrack.addEventListener('touchstart', function(e) {
            startX = e.touches[0].clientX;
            isDragging = true;
            stopAutoPlay();
        });
        
        catalogTrack.addEventListener('touchmove', function(e) {
            if (!isDragging) return;
            e.preventDefault();
        });
        
        catalogTrack.addEventListener('touchend', function(e) {
            if (!isDragging) return;
            ...
            if (Math.abs(diffX) > 50) { // Минимальное расстояние для свайпа
                if (diffX > 0) {
                    nextCatalogSlide(); // Свайп влево - следующий слайд
                } else {
                    prevCatalogSlide(); // Свайп вправо - предыдущий слайд
                }
            }
...
<?php echo $__env->make('layouts.tall', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\laravel\resources\views/index.blade.php ENDPATH**/ ?>